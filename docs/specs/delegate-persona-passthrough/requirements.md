...
2. THE `spawn_child_inner` production adapter SHALL merge the translated `KIRO_AGENT` value into `runtime_env` AFTER `build_session_runtime_env` returns AND BEFORE calling `manager.spawn_agent(...)`. The ordering constraint is that `spawn_agent` takes `runtime_env` BY VALUE, so a merge performed after that call can never reach the child — this invariant SHALL be enforced by a test that goes red when the merge is removed or moved past the call.
   - **NOTE (corrected 2026-08-04)**: an earlier draft of this AC claimed the merge had to precede `apply_kiro_env_policy` "otherwise the policy strips `KIRO_*`". That causal claim is false. `apply_kiro_env_policy(&mut merged_env, runtime_env)` strips `KIRO_*` from the CHILD PROCESS env (`merged_env: Vec`) and borrows `runtime_env` immutably — it cannot unset what `kiro_launch_args(runtime_env)` reads. Stripping `KIRO_AGENT` from the child env is CORRECT and is asserted by the pre-existing `connection.rs` test `kiro_env_policy_strips_codeg_launch_knobs_from_the_child`: `KIRO_AGENT` is a codeg-side knob translated into argv, not an environment variable `kiro-cli` reads.
