...
export function MessageInput({
  ...
  const actionButtons = isEditingQueueItem ? (
    ...
    // [merge-v0.23.0] union: upstream 的 native-steering 分支 (三键 Stop + Send +
    // steer dropdown) 覆盖 onSteer/onEnqueue 齐备场景; fallback 分支保留 HEAD R3.1
    // 的 "Stop + Send 共存" 行为 (原 upstream 只 Stop),避免锁死用户中途 enqueue。
    onSteer && onEnqueue && hasSendableContent ? (
      ...
      // [merge-v0.23.0] HEAD R3.1 fallback: Stop and send COEXIST while the
      // agent is working. The stop button used to REPLACE send here, which
      // locked the user out of the conversation for the whole turn.
      <div className="flex items-center gap-1">
        <Button
          onClick={onCancel}
          variant="destructive"
