...
/// Kiro's GLOBAL scope only — the scope codeg writes. Agent- and Project-scope
/// entries are read-only and surface through `read_kiro_scoped_view`, not
/// through the cross-agent scan (listing them here would offer to unbind
/// entries codeg must not touch).
///
/// A denied credential gate is the EXPECTED default over HTTP, not a broken
/// config, so it reports "no Kiro entries" rather than travelling on as a scan
/// warning: a warning here would both cry wolf in the panel and — since
/// [`require_complete_scan`] refuses to write while any source is unread — wedge
/// every other agent's writes behind a refusal that is working as designed.
/// A real read failure still degrades into a warning like any other source.
fn read_kiro_servers_for_scan() -> LocalMcpReadResult {
...
fn local_mcp_readers() -> [LocalMcpReader; 15] {
    [
        ...
        LocalMcpReader::new("Kiro", McpAppType::Kiro, read_kiro_servers_for_scan),
