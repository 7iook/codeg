...
use crate::acp::types::{
    ...
    SessionFailureRecord, SessionModeInfo, SessionModeStateInfo, SteerOutcome, ToolCallImageInfo,
    UserMessageBlock,
};
...
pub fn kiro_launch_args(runtime_env: &BTreeMap<String, String>) -> Vec<String> {
    ...
    match value(KIRO_TRUST_MODE_ENV)
...
fn apply_kiro_env_policy(
    ...
    for key in KIRO_API_KEY_ENVS {
        let cleared_in_panel = runtime_env.get(*key).is_some_and(|v| v.trim().is_empty());
        let has_value = merged.iter().any(|(k, v)| k == key && !v.trim().is_empty());
...
fn sync_antigravity_settings_file(
    ...
    match write_antigravity_settings(&acp_dir, &path, &updated) {
        ...
        Err(err) => tracing::warn!("[ACP][Antigravity] cannot write {}: {err}", path.display()),
    }
}
...
// [merge-v0.23.0] Removed local `pub enum SteerOutcome` here — it collided
// with upstream's `crate::acp::types::SteerOutcome` (auto-merge kept both).
// The types::SteerOutcome now carries the union of both variant sets:
// Injected / PromptRequired (HEAD) / StartedNewTurn / Failed / Unknown (upstream).
// All local call sites (`SteerOutcome::PromptRequired` etc.) route through the
// existing `use super::types::SteerOutcome;` import in this file's prelude.
...
pub enum ConnectionCommand {
    ...
    // [merge-v0.23.0] Removed the duplicate text-only `Steer { text, reply }` variant
    // that upstream added — it collided with HEAD's structured `Steer { blocks, message_id, reply }`
    // above (auto-merge silently added both, would fail to compile with "duplicate enum variant").
    // HEAD's variant is a strict superset: text can be expressed as a single PromptInputBlock::Text.
    // The manager's cancellation-shielded task + FeedbackSubmitted broadcast lives at the
    // manager layer (submit_feedback), not on this command loop.
    Disconnect,
}
...
async fn run_conversation_loop<'a>(
    ...
    loop {
        ...
        match cmd {
                    ...
                    tokio::select! {
                        ...
                        cmd = cmd_rx.recv() => {
                            match cmd {
                                ...
                                // inline `Steer { text, reply }` +
                                // `send_steer_request` arm — HEAD dispatches
                                // steering OFF the command loop precisely so
                                // a hung `_session/steering` cannot block
                                // Cancel / Disconnect (which upstream's
                                // inline `.await` does). Same site also
            ...
            // [merge-v0.23.0] union: same choice as the in-turn arm above —
            // HEAD's off-loop `spawn_steering_request` with structured `blocks`
            // + `message_id` replaces upstream's inline `NoActiveTurn` reply.
            // HEAD's path still short-circuits (agent answers `startedNewTurn`
            // or `noActiveTurn` — both surface through the same oneshot), so
            // the caller's fallback semantics are preserved without dropping
            // the reply. Same site keeps HEAD's authorized-cancel variant.
        ...
        "subagent_spawned" => {
            ...
            if cb_state.grok_settled_spawn_ids.contains(&call_id) {
                ...
                events.push(AcpEvent::BackgroundActivity {
                    ...
                    outstanding_agents: outstanding(cb_state),
...
mod tests {
    ...
    fn kiro_env_policy_strips_codeg_launch_knobs_from_the_child() {
        ...
        assert!(
            !merged
                .iter()
                .any(|(k, _)| k == KIRO_MODEL_ENV || k == KIRO_AGENT_ENV),
    ...
    fn antigravity_settings_merge_preserves_foreign_keys_and_skips_no_op_writes() {
        ...
        let merged =
            merge_antigravity_settings(Some(existing.clone()), "oauth-business", None, None)
                .expect("editable")
