...
pub enum SteerOutcome {
    ...
    Injected,
    /// [merge-v0.23.0] From HEAD/kiro-agent: the turn we meant to steer had
    /// already settled and the adapter honored the `idleBehavior = "promptRequired"`
    /// opt-in — NOT consumed, still host-owned. The caller reroutes it through
    /// `session/prompt`. This variant covers the same design intent as the
    /// upstream "Failed" variant below (definitively-not-accepted) but for the
    /// specific opt-in path where the adapter cooperated; keep both.
    PromptRequired,
