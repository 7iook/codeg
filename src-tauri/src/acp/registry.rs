...
impl AcpAgentMeta {
    ...
    ///
    /// `SystemBinary` is `false` for a stronger reason: the user owns the
    /// install and codeg has no fetch channel at all (no URL, no cache, no
    /// pinned `registry_version` — see
    /// docs/architecture/ADR-0001-agent-distribution-system-binary.md).
    /// Accepting a requested version would record one codeg never fetched and
    /// cannot fetch, while the process on PATH keeps whatever version the user
    /// installed — the same "reports a build that was never fetched" failure
    /// this predicate exists to prevent, in its most absolute form.
    pub fn supports_custom_version(&self) -> bool {
        match &self.distribution {
            AgentDistribution::Npx { .. } => true,
            AgentDistribution::Uvx { .. } => false,
            AgentDistribution::SystemBinary { .. } => false,
...
// gate:allow-unwired consumed by connection.rs synthesize_native_steering version gate
...
fn distribution_uses_cursor_acp(distribution: &AgentDistribution) -> bool {
    match distribution {
        // `SystemBinary` launches `cmd` straight off PATH, so its (cmd, args)
        // pair IS the resolved launch recipe — exactly what the Npx / Binary
        // arms inspect. A user-installed `cursor-agent … acp` must advertise
        // the same client capabilities as the built-in entry that wraps the
        // same binary; where the executable came from is irrelevant to what
        // protocol it speaks.
...
mod tests {
    ...
    fn all_acp_agents_covers_every_enum_variant() {
        ...
        assert!(agents.contains(&AgentType::DeepSeek));
        assert!(agents.contains(&AgentType::Qoder));
        assert!(agents.contains(&AgentType::Antigravity));
