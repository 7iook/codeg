...
// [merge-v0.23.5] union: HEAD's continue/close-session transport + upstream's
// task_progress/task_complete and the new create_automation/create_work_task.
...
use crate::acp::delegation::transport::{
    ...
    BrokerCancelTaskRequest, BrokerCloseSessionRequest, BrokerCommitFeedbackRequest,
    BrokerContinueRequest, BrokerCreateAutomationRequest, BrokerCreateWorkTaskRequest,
...
impl CompanionFeatures {
    ...
    pub fn allows_tool(&self, name: &str) -> bool {
        match name {
            ...
            // [merge-v0.23.5] union: HEAD's continue_with_session/close_session (delegation group)
            // + upstream's task_progress/task_complete (tasks) and
            // create_automation/create_work_task (their own chat-authoring groups).
...
mod tests {
    ...
    #[tokio::test]
    async fn tools_list_returns_six_delegation_tools() {
        ...
        let resume_required = resume["inputSchema"]["required"].as_array().unwrap();
        assert_eq!(resume_required.len(), 1);
        assert!(resume_required.iter().any(|v| v == "task_id"));
