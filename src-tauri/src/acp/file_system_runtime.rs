...
fn agent_root_slots(agent_type: AgentType) -> &'static [RootSlot] {
    match agent_type {
        ...
        AgentType::Kiro => &[RootSlot {
            // `resolve_kiro_home_from` is a bare `PathBuf::from` on the env
            // value — no `expanduser` — so the value stays VERBATIM.
            candidates: &[("KIRO_HOME", "sessions", VERBATIM)],
            trims: false,
...
mod tests {
    ...
    /// No relocation value may be tilde-expanded for the agents whose own
    /// resolver takes the value verbatim: they treat `~` literally, so expanding
    /// it here would hand out `$HOME` as a writable root the user never
    /// selected. Hermes' contract is the explicit one — `get_hermes_home` is
    /// `Path(val.strip())`, no `expanduser`, which
    /// `commands::acp::hermes_home_for_launch` mirrors — and a BLANK value falls
    /// back to `~/.hermes` rather than re-inheriting the parent.
    ///
