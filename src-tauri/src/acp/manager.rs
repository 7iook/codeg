            ...
            let handle = tokio::spawn(async move {
                ...
                let outcome: Result<(), AcpError> = async {
                    ...
                    let preserved = if let Some(eid) = eid_opt {
                        // Requirement 3.3a: Branch A continuation may carry a
                        // fallback `session/new` id. A Delegate row's existing
                        // credential must win — `bind_external_id` would split
                        // that history onto a sibling.
                        let skip_bind = match conversation_service::get_by_id_optional(
                            &db_conn,
                            linked_conversation_id,
                        )
                        .await
                        {
                            Ok(Some(row)) if row.kind == ConversationKind::Delegate => {
                                matches!(
                                    row.external_id.as_deref(),
                                    Some(existing) if !existing.is_empty() && existing != eid
                                )
                            }
                            Ok(_) => false,
                            Err(e) => return Err(AcpError::protocol(e.to_string())),
                        };
                        if skip_bind {
                            tracing::info!(
                                conversation_id = linked_conversation_id,
                                incoming = %eid,
                                "[manager] refusing to overwrite delegate row's resume \
                                 credential (incoming id looks like a session/new \
                                 fallback — Requirement 3.3a)"
                            );
                            None
                        } else {
        ...
        let handle = tokio::spawn(async move {
            let outcome: Result<FeedbackItem, AcpError> = async {
                ...
                cmd_tx
                    .send(ConnectionCommand::Steer {
                        blocks: wire_blocks,
                        // [merge-v0.30.0] HEAD's Steer variant additionally carries a
                        // `message_id` for log correlation; upstream's has none, so mint
                        // one here. The blocks themselves now come from upstream's
                        // `wire_blocks`, which already handles the attachment case.
                ...
                match steer {
                    ...
                    // [merge-v0.23.0] Upstream added two more variants; treat both as
                    // definitely-not-accepted so the frontend's normal-prompt fallback engages.
                    // Failed = agent explicitly refused (safe to re-queue). Unknown = transport
                    // broken / timeout (per design §2.5.1 upstream doc, NEVER auto-retry — but
                    // surfacing NoActiveTurn to caller lets it show "delivery result unknown").
                    SteerOutcome::Failed | SteerOutcome::Unknown => {
                        return Err(AcpError::NoActiveTurn);
                    }
                }
...
mod tests {
    ...
    fn answer_steer(
        ...
        tokio::spawn(async move {
            match rx.recv().await {
                // `..` skips HEAD's extra `message_id` field on the variant.
