    ...
    async fn submit_feedback_native(
        ...
        let handle = tokio::spawn(async move {
            let outcome: Result<FeedbackItem, AcpError> = async {
                ...
                cmd_tx
                    .send(ConnectionCommand::Steer {
                        // [merge-v0.23.0] Adapt upstream's text-only submit_feedback
                        // to HEAD's structured Steer variant: wrap in single Text block,
                        // mint a message_id for log correlation.
