...
impl PromptLedger {
    ...
    /// `connection.rs`).
    ///
    /// [merge-v0.23.4] Test-only now: the production `Steer` arms fingerprint
    /// through [`Self::record_prompt_blocks`] directly (they already hold the
    /// structured `blocks`), so this bare-string wrapper survives only as the
    /// terse form the watcher tests use to stage a steered record.
    #[cfg(test)]
