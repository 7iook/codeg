    ...
    match &envelope.payload {
        ...
        AcpEvent::SessionStarted { session_id } => {
            ...
            if let Some(cid) = conversation_id {
                // Requirement 3.3a: a Delegate child's resume credential must
                // not be overwritten by a differing session id (that shape is
                // the resume chain's silent `session/new` fallback).
                // `bind_external_id` would split that history onto a sibling.
                // Root / other rows take the codeg#500 guarded bind.
                let skip_bind = match conversation_service::get_by_id_optional(db_conn, cid).await {
                    Ok(Some(row)) if row.kind == ConversationKind::Delegate => {
                        matches!(
                            row.external_id.as_deref(),
                            Some(existing) if !existing.is_empty() && existing != session_id
                        )
                    }
                    Ok(_) => false,
                    Err(e) => return Err(e),
                };
                let preserved = if skip_bind {
                    tracing::info!(
                        conversation_id = cid,
                        incoming = %session_id,
                        "[lifecycle] refusing to overwrite delegate row's resume \
                         credential (incoming id looks like a session/new fallback — \
                         Requirement 3.3a)"
                    );
                    None
                } else {
